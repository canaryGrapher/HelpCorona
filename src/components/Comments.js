...
const Comments = (props) => {
  ...
  const icon_for_like = !liked ? (
    <span
      style={{ lineHeight: "30px" }}
      onClick={() => likes(props.identifier)}
    >
    ...
    <span style={{ lineHeight: "30px" }}>
      <LikeFilled className="my-auto mr-1 text-primary" />
      <span className="comment-action my-auto">{commentLikes}</span>
    </span>
  );
  const icon_for_dislike = !disliked ? (
    <span
      style={{ lineHeight: "30px" }}
      onClick={() => dislikes(props.identifier)}
    >
    ...
    <span style={{ lineHeight: "30px" }}>
      <DislikeFilled className="my-auto mr-1 text-primary" />
      <span className="comment-action my-auto">{commentDislikes}</span>
    </span>
  );

  const actions = [
    <Tooltip key="comment-basic-like" className="text-dark" title="Useful">
      {icon_for_like}
    </Tooltip>,
    <Tooltip
  ...
  return (
    <Comment
      ...
      content={
        <div>
          <p className="text-muted">{random.place() + " " + random.last()}</p>
          <p>{props.content}</p>
        </div>
      }
      datetime={
        <mark className="text-white" style={{ backgroundColor: "#89b7f3" }}>
